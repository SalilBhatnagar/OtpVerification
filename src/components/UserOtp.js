...
export default function SignIn() {
    ...
    // Validate OTP
    const ValidateOtp = () => {
        if (OTP === null || optTrack === null)
            return;
            optTrack.confirm(OTP).then((result) => {
                setMesError(false)
            alert(" success");
        }).catch((err) => {
            //alert("Wrong code");
    ...
    function otpSend()  {
       ...
       console.log(" opt checking")
        let verify = new firebase.auth.RecaptchaVerifier('recaptcha-container');
        auth.signInWithPhoneNumber(currentMobile, verify).then((result) => {
            console.log(result);
            setOptTrack(result);
    ...
    return (
        <>
          {!currentMobile ? history.push("/VarificationForm"):
